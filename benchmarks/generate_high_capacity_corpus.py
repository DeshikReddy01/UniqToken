...
def generate_high_capacity_multilingual_corpus(
    ...
    for lang, (chars, suffixes, prefixes) in scripts.items():
        ...
        if suffixes and prefixes:
            extra_both = [
                pre + w + suf
                for w in raw_words[:3000]
                for pre in rng.sample(prefixes, k=1)
                for suf in rng.sample(suffixes, k=1)
            ]
        ...
        for _ in range(num_docs):
            ...
            if "English" in lang:
                ...
                if rng.random() < 0.35:
                    w_sample.append(
                        f"SELECT {rng.choice(['col_a', 'col_b', 'idx_id'])} FROM table_{rng.randint(1, 999)} WHERE ts > {rng.randint(10000, 99999)};"
                    )
                if rng.random() < 0.30:
                    w_sample.append(f"0x{rng.randint(0, 0xFFFFFFFF):08x}")
                if rng.random() < 0.30:
                    w_sample.append(
                        f"uuid_{rng.randint(1000, 9999):x}-{rng.randint(100, 999):x}-{rng.randint(1000, 9999):x}"
                    )
...
if __name__ == "__main__":
    ...
    print("Target Max Vocabulary           : 65,536")
