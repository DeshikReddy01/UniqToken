...
print("=" * 140)
print(f"{'Scale (Req)':<12} | {'Seed':<6} | {'Model Architecture':<30} | {'True BPB':<10} | {'Token CE':<10} | {'B/Tok':<8} | {'Active %':<10} | {'>=6B %'}")
print("-" * 140)

for r in records:
    print(f"V={r['vocab_size']:<10} | {r['seed']:<6} | {r['model_name']:<30} | {r['true_lm_bpb']:<10.3f} | {r['token_ce_loss']:<10.3f} | {r['bytes_per_token']:<8.2f} | {r['active_vocab_pct']:<10.1f}% | {r['pct_ge_6b']:<6.1f}%")
print("=" * 140)
